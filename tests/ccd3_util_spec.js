...
describe("ccd3.Util.", function(){
	it("dataset_from_array work.",function(){
		...
		dataset = ccd3.Util.dataset_from_array(ar,[{x:0,y:1},{x:0,y:2}]);
		...
		dataset = ccd3.Util.dataset_from_array(ar,[{x:0,y:1},{x:0,y:2}],{header_row:true});
		expect(dataset[0].values[0].x).toEqual("a");
		expect(dataset[0].values[0].y).toEqual(1);
		...
		dataset = ccd3.Util.dataset_from_array(ar,[{x:"label1",y:"label2"},{x:"label1",y:"label3"}]);
		expect(dataset[0].values[0].x).toEqual("a");
		expect(dataset[0].values[0].y).toEqual(1);
		expect(dataset[0].values[4].x).toEqual("e");
		expect(dataset[0].values[4].y).toEqual(5);
		...
		dataset = ccd3.Util.dataset_from_array(ar,
		...
		var chart = new ccd3.Chart("chart"+seq,dataset);
		chart.render();
		seq++;
		*/
